  ...
  <div class="chart-editor">
    ...
    <div class="editor-header">
      ...
      <div class="editor-toolbar">
        ...
        <button @click="deleteSelectedFxEvent" :disabled="selectedFxEventIndex === null" class="delete-button">Delete FX Event</button>
      ...
      <div class="editor-content">
        <div v-if="localChart" class="timeline-container">
          ...
          <EditorTimeline
             ...
             @selectFxEvent="selectFxEvent"
             @updateFxEvent="handleUpdateFxEvent"
        />
        </div>
        <div v-else>
    ...
    <div v-if="localChart" class="editor-sidebar">
      ...
      <div v-if="selectedCameraEvent" class="sidebar-section">
       ...
       <div class="sidebar-section">
        <h3>FX Events</h3>
        <button @click="addFxEvent">Add FX Event</button>
        <div class="event-list">
          <div
            v-for="(event, index) in localChart.fxEvents"
            :key="index"
            class="event-item"
            :class="{ selected: selectedFxEventIndex === index }"
            @click="selectFxEvent(index)"
          >
            {{ index }}: {{ event.time }}ms - {{ event.type }}
          </div>
        </div>
      </div>
      <div v-if="selectedFxEvent" class="sidebar-section">
        <h3>FX Event Properties</h3>
        <div class="property">
          <label for="fx-event-time">Time (ms)</label>
          <input id="fx-event-time" type="number" v-model.number="selectedFxEvent.time">
        </div>
        <div class="property">
          <label for="fx-event-type">Type</label>
          <select id="fx-event-type" v-model="selectedFxEvent.type">
            <option value="lineBurst">Line Burst</option>
          </select>
        </div>
        <div class="property">
          <label for="fx-event-duration">Duration (ms)</label>
          <input id="fx-event-duration" type="number" v-model.number="selectedFxEvent.duration">
        </div>
        <div class="property">
          <label for="fx-event-color">Color</label>
          <input id="fx-event-color" type="text" v-model="selectedFxEvent.color">
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    selectedFxEvent() {
      if (this.selectedFxEventIndex === null || !this.localChart || !this.localChart.fxEvents) return null;
      return this.localChart.fxEvents[this.selectedFxEventIndex];
    },
    playheadStyle() {
  ...
  methods: {
    async loadChart() {
      try {
        ...
        if (!this.localChart.fxEvents) {
          this.localChart.fxEvents = [];
        }
    ...
    handleSelectNote(noteId) {
      if (this.selectedNoteId === noteId) {
        ...
        this.selectedFxEventIndex = null;
      }
    },
    ...
    addFxEvent() {
      ...
      this.localChart.fxEvents.push(newEvent);
      this.localChart.fxEvents.sort((a, b) => a.time - b.time);
      this.selectFxEvent(this.localChart.fxEvents.indexOf(newEvent));
    },
    selectFxEvent(index) {
      if (this.selectedFxEventIndex === index) {
        this.selectedFxEventIndex = null;
      } else {
        this.selectedFxEventIndex = index;
        this.selectedNoteId = null;
        this.selectedEventIndex = null;
        this.selectedCameraEventIndex = null;
      }
    },
    deleteSelectedFxEvent() {
      if (this.selectedFxEventIndex !== null) {
        this.localChart.fxEvents.splice(this.selectedFxEventIndex, 1);
        this.selectedFxEventIndex = null;
      }
    },
    ...
    handleUpdateFxEvent(updatedEvent) {
      if (!this.localChart || !this.localChart.fxEvents) return;
      const event = this.localChart.fxEvents[updatedEvent.index];
      if (event) {
        event.time = updatedEvent.time;
        this.localChart.fxEvents.sort((a, b) => a.time - b.time);
      }
